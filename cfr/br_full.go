...
type BestResponseFull struct {
	*Trainer
}

func (trainer *BestResponseFull) String() string {
	return string(BR_FULL_T)
}

func (trainer *BestResponseFull) regretUpdateEquation(
	t int,
	regret float32,
...
func (trainer *BestResponseFull) strategyUpdateEquation(
	t int,
	reach_prob float32,
...
func (trainer *BestResponseFull) Train(profile IProfile) {
...
func (trainer *BestResponseFull) run(
	profile IProfile,
	p *pdt.Partida,
	acc []float32,
	update_player int,
	...
	if esTurnoDelAgente {
		...
		pkts := a.Hacer(p)
		termino := p.Terminada()
		pts_ganados, ganador := utils.IsDoneAndPtsFull(pkts)
